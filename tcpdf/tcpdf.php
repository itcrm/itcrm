...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF extends DBObject {
		 ...
		 * @param string $family Family font. It can be either a name defined by AddFont() or one of the standard Type1 families (case insensitive):<ul><li>freesans</li></ul> It is also possible to pass an empty string. In that case, the current family is retained.
		...
		protected function _putfonts() {
			...
			foreach ($this->fontkeys as $k) {
				...
				if ($type == 'core') {
					...
					if (strtolower($name) == 'freesans') {
						// add default font for annotations
						$this->annotation_fonts['freesans'] = $k;
					}
					$out .= ' >>';
					$out .= "\n".'endobj';
					$this->_out($out);
		...
		protected function _putcatalog() {
			...
			if (!empty($this->form_obj_id) OR ($this->sign AND isset($this->signature_data['cert_type']))) {
				...
				$out .= ' /DA (/F'.(array_search('freesans', $this->fontkeys) + 1).' 0 Tf 0 g)';
				$out .= ' /Q '.(($this->rtl)?'2':'0');
				//$out .= ' /XFA ';
				$out .= ' >>';
				// signatures
