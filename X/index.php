...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    $txt = $_POST['txt'];
    $pass = $_POST['pass'];
    $c = new McryptCipher($pass);
    if ($_POST['cmb'] == 'encrypt') {
        $res = $c->encrypt($txt);
    } else {
        $res = $c->decrypt($txt);
    }
    ...
    <form id="frm1" method="post">
        <table width="50%" border="1" cellspacing="5" cellpadding="5">
            <tbody>
                <tr>
                    <td>***</td>
                    <td><input name="pass" type="password" id="pass" tabindex="1" value="<?= $pass; ?>" size="100%"></td>
                </tr>
                <tr>
                    <td>txt</td>
                    <td><textarea name="txt" cols="120" rows="5" id="txt" tabindex="2"><?= $txt; ?></textarea></td>
                </tr>
                <tr>
                    <td>$</td>
                    <td><textarea name="res" cols="120" rows="5" id="res" tabindex="2"><?= $res; ?></textarea></td>
                </tr>
                <tr>
                    <td>
                        <select name="cmb" id="cmb">
                            <option selected value="encrypt">+</option>
                            <option value="dencrypt">-</option>
                        </select>
                    </td>
                    <td><input type="button" name="btnClear" id="btnClear" value="x">
                        <input name="submit" type="submit" id="submit" tabindex="3" value="=">
                    </td>
                </tr>
            </tbody>
        </table>
